...
Built-in identity handlers are inferred for the Google and GitHub presets.
For other providers, pass a custom `identity_handler` to `OAuthClient`.
OAuth protocol endpoints come from the provider `preset(...)` config; identity
API endpoints are provider-specific internals handled by the identity handler.
